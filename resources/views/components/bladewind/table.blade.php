...
@endphp
<div
    class="@if ($has_border) border border-gray-200 dark:border-dark-700/60 dark:px-[1px] @endif max-w-full">
    <div class="w-full">
        @if ($searchable)
            <div class="bw-table-filter-bar">
                <x-bladewind::input name="bw-search-{{ $name }}" placeholder="{{ $search_placeholder }}"
                    ...
                    class="!mb-0 focus:!border-slate-300 !pl-11" clearable="true" prefix_is_icon="true"
        ...
        <div class="overflow-x-scroll ">
            <table
                class="bw-table w-full {{ $name }} @if ($has_shadow) drop-shadow shadow-gray-200 dark:shadow-dark-900 @endif
                            ...
                            @foreach ($data as $row)
                                <tr>
                                    ...
                                    @if (!empty($icons_array))
                                        <td class="space-x-2 text-right actions">
                                            @foreach ($icons_array as $icon)
                                                @if (isset($icon['icon']))
                                                    ...
                                                    @endif
                                                    <x-bladewind::button.circle size="tiny"
                                                        icon="{{ $icon['icon'] }}" color="{{ $icon['color'] ?? '' }}"
                                                        {{-- title="{{$icon['tip']??''}}" --}} onclick="{!! build_click($icon['click'], $row) ?? 'void(0)' !!}"
